...
## 1) Workspace map
- `02_DEV_N8N_ARCHITECT/backends/system-ia-agentes/`: main FastAPI backend (active, own git repo)
- `02_DEV_N8N_ARCHITECT/workflows/repo-demo/`: n8n workflow backup repo
- `electronica-web/`: Vite + React demo frontend (secondary)
- `02_DEV_N8N_ARCHITECT/ai-sandbox/pruebas_crewai/`: local CrewAI experimentation sandbox
- `02_DEV_N8N_ARCHITECT/backends/legacy-cerebro-central/`: legacy FastAPI backend
- `memory/`: operational runbooks and process memory
- `.agents/skills/`: n8n-focused skill docs

Architecture rule:
- IG/FB social comment auto-replies are implemented in FastAPI (`02_DEV_N8N_ARCHITECT/backends/system-ia-agentes/workers/social/worker.py`), not by duplicating n8n workflows.
...
Note: `02_DEV_N8N_ARCHITECT/workflows/gh-aw/` is a separate embedded repo with its own AGENTS/conventions; do not import them unless task scope explicitly targets `gh-aw/`.

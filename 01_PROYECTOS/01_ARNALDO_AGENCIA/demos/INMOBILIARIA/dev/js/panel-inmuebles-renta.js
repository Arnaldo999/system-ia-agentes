...
(function () {
  ...
  function _renderTablaInmuebles() {
    ...
    if (!cont) return;
    if (_items.length === 0) {
      cont.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:30px;color:var(--txt-2)">
        Sin inmuebles registrados. Agregá el primero con el botón + Nuevo.
      </td></tr>`;
      return;
    }
  ...
  window.guardarInmueble = async function () {
    ...
    const campos = {
      ...
      propietario_id: parseInt(document.getElementById('inmueblePropietario').value) || null,

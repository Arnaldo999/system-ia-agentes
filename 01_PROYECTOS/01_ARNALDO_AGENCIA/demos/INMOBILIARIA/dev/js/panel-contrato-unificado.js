...
(function () {
  ...
  function _aplicarPreload() {
    ...
    if (p.item_tipo) {
      ...
      if (p.bloqueado) {
        // Deshabilitar los radios de tipo
  ...
  function _renderPaso() {
    ...
    // Cargar datos dinámicos del paso
  ...
  async function _cargarDatosPaso1() {
    ...
    // Cargar leads en estado relevante
    try {
  ...
  function _validarPaso1() {
    ...
    if (!modo) return 'Seleccioná un tipo de cliente';
    if (modo === 'existente') {
      if (!_getVal('cu-cliente-select')) return 'Seleccioná un cliente activo';
    }
    if (modo === 'convertir_lead') {
      if (!_getVal('cu-lead-select')) return 'Seleccioná un lead a convertir';
    }
    if (modo === 'cliente_nuevo') {
      ...
      if (!_getVal('cu-nuevo-tel')) return 'El teléfono es obligatorio';
    }
    return null;
  }
  ...
  function _validarPaso2() {
    if (!_getVal('cu-item-select')) return 'Seleccioná el activo (lote, propiedad o inmueble)';
    return null;
  }
  ...
  function _validarPaso3() {
    if (!_getVal('cu-fecha-firma')) return 'Ingresá la fecha de firma';
    if (!_getVal('cu-monto-total')) return 'Ingresá el monto total';
    if (!_getVal('cu-cuotas-total')) return 'Ingresá las cuotas totales';
    return null;
  }
  ...
  function _construirBody() {
    ...
    const itemId = _getVal('cu-item-select');

    // Determinar item_tipo desde tipo de contrato
    const itemTipoMap = {
      venta_lote: 'lote',
    ...
    const body = {
      ...
      asesor_id: parseInt(_getVal('cu-asesor')) || null,
    ...
    // Cliente según modo
    if (modo === 'existente') {
      body.cliente_activo_id = parseInt(_getVal('cu-cliente-select')) || null;
    } else if (modo === 'convertir_lead') {
      body.convertir_lead_id = _getVal('cu-lead-select');
  ...
  window._cuCerrar = function () {
    document.getElementById('modalContratoUnificado').classList.remove('show');
    // Rehabilitar radios de tipo (por si venían bloqueados desde mapa)

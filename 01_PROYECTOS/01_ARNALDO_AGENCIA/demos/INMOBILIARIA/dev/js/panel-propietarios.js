// Panel Propietarios — autocomplete persona única + rol propietario
(function(){
  let PROPIETARIOS = [];
  ...
  window.abrirModalPropietario = function(data) {
    ...
    // Si es nuevo, mostrar bloque autocomplete; si edición, ocultarlo
  ...
  window.propBuscarPersona = function(input) {
    ...
    _autocompleteTid = setTimeout(async () => {
      try {
        ...
        if (items.length === 0) {
          lista.innerHTML = '<div style="padding:8px;color:var(--txt-2);font-size:12px">Sin resultados — completá los campos manualmente.</div>';
        } else {
  ...
  window.guardarPropietario = async function() {
    ...
    try {
      if (id) {
        ...
        // Crear en tabla propietarios igual (para mantener compat con inmuebles_renta.propietario_id)
        ...
        await crmCreate('propietarios', campos);
        // Intentar crear cliente_activo también con rol propietario
        try {

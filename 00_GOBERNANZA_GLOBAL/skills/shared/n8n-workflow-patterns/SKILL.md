...
6. **Sub-Workflow Execution** (Reusability Pattern)
   - Call child workflows from parent workflows
   - Pattern: Trigger → Execute Workflow → Process Result

7. **Error Handling Workflow** (Reliability Pattern)
   - Catch and handle errors from any workflow
   - Pattern: Error Trigger → Notify → Log → Retry
...
### Example 6: Sub-Workflow (Reusable Logic)

**Parent workflow**:
```
1. Webhook (trigger)
2. Execute Workflow (calls "Process User" sub-workflow)
3. Set (use result)
4. Respond to Webhook
```

**Child workflow ("Process User")**:
```
1. Execute Workflow Trigger (receives data from parent)
2. HTTP Request (enrich user data)
3. Code (transform)
4. Return to parent
```
...
{
  "workflowId": "{{$vars.PROCESS_USER_WORKFLOW_ID}}",
  "mode": "passthrough",  // or "each" to run once per item
  "inputData": {
...
```
1. Error Trigger (catches errors from other workflows)
2. Set (format error info)
3. Slack (notify #alerts channel)
4. Postgres (log error to database)
```

**Activate error workflow**:
- Go to broken workflow → Settings → Error Workflow → Select your error handler

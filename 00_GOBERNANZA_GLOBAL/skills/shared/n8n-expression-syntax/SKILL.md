...
{{$execution.resumeUrl}}    // URL for Wait node callbacks
```
...
**Use case**: Add position to each item:
```
Item #{{$itemIndex + 1}} of total
```

### $runIndex - Loop Run Counter

```javascript
{{$runIndex}}               // How many times this node has run (0-based)
```

**Use case**: Batch numbering inside loops:
```
Batch {{$runIndex + 1}}
```

### $items() - Get Items from Named Node

```javascript
{{$items("Webhook")[0].json.body.name}}
{{$items("HTTP Request")[0].json.data.id}}
{{$items("IF", 0, 0)[0].json.result}}    // node, outputIndex, runIndex
```
...
|----------|-------------------|---------|
| `$json` | Current node output | `{{$json.name}}` |
| `$node["Name"].json` | Other node output | `{{$node["Webhook"].json.body.email}}` |
| `$now` | Current datetime (Luxon) | `{{$now.toFormat('yyyy-MM-dd')}}` |
| `$today` | Today's date at midnight | `{{$today.toFormat('yyyy-MM-dd')}}` |
| `$env` | Environment variables | `{{$env.API_KEY}}` |
| `$vars` | Workflow static variables | `{{$vars.MAX_RETRIES}}` |
| `$workflow.id` | Workflow ID | `{{$workflow.id}}` |
| `$workflow.name` | Workflow name | `{{$workflow.name}}` |
| `$execution.id` | Execution ID | `{{$execution.id}}` |
| `$execution.mode` | How it was triggered | `{{$execution.mode}}` |
| `$execution.resumeUrl` | Wait node callback URL | `{{$execution.resumeUrl}}` |
| `$itemIndex` | Current item index (0-based) | `{{$itemIndex}}` |
| `$runIndex` | Current loop run (0-based) | `{{$runIndex}}` |
| `$items("Node")[0].json` | Items from named node | `{{$items("HTTP Request")[0].json.id}}` |
| `$secrets.KEY` | Secure credential value | `{{$secrets.API_KEY}}` |

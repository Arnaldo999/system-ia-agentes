...
## Complete n8n Variables Reference

### $vars - Workflow Static Variables

Set in **Workflow Settings → Variables**. Useful for config values shared across nodes.

```javascript
// Access static workflow variables
const apiUrl = $vars.API_URL;
const environment = $vars.ENVIRONMENT; // 'production', 'staging', etc.
const maxItems = Number($vars.MAX_ITEMS) || 100;
...
return [{ json: { workflowInfo: info } }];
```

### $execution - Execution Context

```javascript
// Access current execution data
const execId = $execution.id;
const mode = $execution.mode;         // 'manual', 'trigger', 'webhook', 'retry'
const resumeUrl = $execution.resumeUrl; // For Wait node integrations

return [{
  json: {
    executionId: execId,
    triggeredBy: mode,
    callbackUrl: resumeUrl || null
  }
}];
```
...
Available in "Run Once for All Items" when iterating with `.forEach` or in expressions:
...
### $runIndex - Loop Iteration Counter

Useful inside loops (Split in Batches → Code):

```javascript
// Tracks how many times this node has run in current execution
const currentRun = $runIndex; // 0, 1, 2...

return [{
  json: {
    batchNumber: currentRun + 1,
    isFirstBatch: currentRun === 0
  }
}];
```

### $items() - Get Items from Specific Node

```javascript
// Get output items from any node by name
const webhookItems = $items("Webhook");
const httpItems = $items("HTTP Request");

// With output index and run index (for multi-output nodes)
const trueItems = $items("IF", 0, 0);  // true branch output
const falseItems = $items("IF", 1, 0); // false branch output

const webhookBody = webhookItems[0].json.body;
return [{ json: { data: webhookBody } }];
```

### $secrets - Secure Credential Access

```javascript
// Access secrets from n8n credentials store
// Only works if configured in n8n settings
...
```javascript
// Access binary data (files, images, PDFs) from previous node
const item = $input.first();
const binaryFields = item.binary; // Object with binary fields

// Check if binary data exists
if (!binaryFields || !binaryFields.data) {
  return [{ json: { error: 'No binary data received' } }];
}

// Get metadata
const mimeType = binaryFields.data.mimeType;
const fileName = binaryFields.data.fileName;
const fileSize = binaryFields.data.fileSize;
...
```javascript
// Create a file as binary output (e.g., generated CSV)
const csvContent = "name,email\nJohn,john@example.com\nJane,jane@example.com";
const base64Data = Buffer.from(csvContent).toString('base64');

return [{
  json: { success: true, rows: 2 },
  binary: {
    data: {
      data: base64Data,
      mimeType: 'text/csv',
...
### Process Binary + Return JSON

```javascript
// Read binary, process, output result as JSON
const item = $input.first();
...
if (binaryData) {
  ...
  return [{
    json: {
      originalFile: binaryData.fileName,
...
return [{ json: { error: 'No binary attachment' } }];
```

---

## pairedItem - Track Item Origins for Merging

Essential when using **Merge** node or when output must link back to input:
...
return items.map((item, index) => ({
  json: {
    result: `Processed: ${item.json.name}`,
    status: 'done'
  },
  pairedItem: {
    item: index  // Links output back to input item at this index
  }
}));
```

```javascript
// For single output that aggregates multiple inputs
const items = $input.all();
const combined = items.map(i => i.json.value).join(', ');
